...
allxy=(${paygatewayxy[@]} ${paytoolxy[@]} ${wallstreetxy[@]} ${irouterxy[@]} ${paysmsxy[@]} ${paycap[@]})
allyz=(${paygatewayyz[@]} ${paytoolyz[@]} ${wallstreetyz[@]} ${irouteryz[@]} ${paysmsyz[@]})
...
searchByTraceId() {
  ...
  do
    ...
    echo "==========================================="
    ssh -i /home/user/caoss/key/readonly-zhifu readonly@$ipb "grep --color '$traceid' /data/logs/irouter/root.$day*"
    echo
  done
}
